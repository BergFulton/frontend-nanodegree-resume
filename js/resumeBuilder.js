var bio = {
    ...
    // including display function within the object declaration, despite JSLint 
    //not validating it (unless the function is all on one line)
    ...
    // I previously did this in an alternative manner w/valid JSON - see this 
    //commit: https://github.com/BergFulton/frontend-nanodegree-resume/commit/b80f12872c174f53f85d6674f8dc1939dafe7cc4
    'display': function() {
...
var work = {
    'jobs': [{
        ...
        'description': 'Provenance researcher and data lead for the'+
            '<a class="project-link" href="http://blog.cmoa.org/category/series/art-tracks/">'+
            ' Art Tracks project.'
    }, {
        'employer': 'Shoefitr',
        'title': 'Data Manager',
        ...
        'description': 'Responsible for all collections activity, loans,'+
        ...
        'dates': '2011-2012',
        'description': 'Collections management, arts management, and exhibition'
        + 'work for a variety of private clients and organizations.'
    }, {
        ...
        'dates': '2010-2011',
        'description': 'Assist with all aspects of art collection stewardship,'+ 
        'opening of new wing, and intake of new collection.'
    }],
    // including display function within the object declaration, despite JSLint 
    //not validating it (unless the function is all on one line)
    // https://discussions.udacity.com/t/json-and-display-function/26209
    ...
    'display': function() {
        for (var i = 0, len = work.jobs.length; i < len; i++) {
            if (work.jobs.hasOwnProperty(i)) {
                ...
                var formattedUrl = $('.work-entry .employer-link')
                    .attr('href', work.jobs[i].url);
            }
        }
    }
};
...
var speaking = {
    'conf': [{
        ...
        'name': 'European Registrars Conference',
        'date': 'December 2012',
        'topic': 'The World Needs to See This!; Making your objects tweet,' + 
            ' buzz, and go viral',
    ...
    'display': function() {
        for (var i = 0, len = speaking.conf.length; i < len; i++) {
            if (speaking.conf.hasOwnProperty(i)) {
                ...
                var formattedName = HTMLspeakingName
                    .replace('%data%', speaking.conf[i].name);
                $('.speaking-entry:last').append(formattedName);

                var formattedLocation = HTMLspeakingLocation
                    .replace('%data%', speaking.conf[i].location);
                $('.speaking-entry:last').append(formattedLocation);

                var formattedDates = HTMLspeakingDates
                    .replace('%data%', speaking.conf[i].date);
                $('.speaking-entry:last').append(formattedDates);

                var formattedTopic = HTMLspeakingTopic
                    .replace('%data%', speaking.conf[i].topic);
...
var education = {
    'schools': [{
        ...
        'dates': '2006',
        'url': 'http://www.otterbein.edu' 
        //yes, this ^ is the right URL. There is no specifc link.
    }],
    'onlineCourses': [{
        ...
        for (var j = 0, OcLen = education.onlineCourses.length; j < OcLen; j++) {
            if (education.onlineCourses.hasOwnProperty(j)) {
                ...
                var formattedTitle = HTMLonlineTitle
                    .replace('%data%', education.onlineCourses[j].title);
                var formattedSchool = HTMLonlineSchool
                    .replace('%data%', education.onlineCourses[j].school);
                $('.education-entry:last').append(formattedTitle + formattedSchool);

                var formattedOnlineDates = HTMLonlineDates
                    .replace('%data%', education.onlineCourses[j].date);
                $('.education-entry:last').append(formattedOnlineDates);

                var formattedUrl = HTMLonlineURL
                    .replace('%data%', education.onlineCourses[j].url)
                    .replace('%url%', education.onlineCourses[j].url);
